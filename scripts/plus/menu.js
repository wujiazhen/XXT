...
(function($){
	...
	Menu.prototype = {
		...
		init : function(options){
			...
			_this.filter_fn = settings.filter_fn || null;	//过滤器
			
			_this.$bg = $('<div class="menu_bg"><div class="menu_bg_left"></div></div>');
		...
		showMenu : function(offset){		
			if (this.$bg.is(':visible')) return;
				
			var parent_width = this.parent.width(),
				menu_width = 156;	//暂定的宽度
			...
			if(overflow){				
				if(this.parent_menu){
					menu_left = menu_left - menu_width*2 + 4; //微调
				}else{
		...
		buildMenu : function(){
			...
			$(document).click(function(e){ clickBind(e);});
			
			function clickBind(e){
				var from = !e.srcElement ? null : e.srcElement.className;
	
				if (from == null || from.indexOf('menu') == -1) {
					_this.hideMenu();
				};
			}
		...
		resetMenu : function(){
			...
			for(var i = 0, l = _this.menu_items.length; i < l; i++){								
				_this.$menu.append(_this.menu_items[i].$item);
				_this.resizeMenu(_this.menu_items[i], 'add');
			}
		...
		resizeMenu : function(menu_item, type){
			...
			ih = menu_item.type == '3' ? 5 : 24;//menu_item.$item.height();
		...
		//设置触发菜单的源，即在源上右键后可显示菜单,所有子菜单的源与顶级菜单的源保持一致
		...
		//过滤符合条件的菜单项，暂时写得比较简单
		filter : function(){
			var fn = this.filter_fn;
			
			for (var i=0, l = this.menu_items.length; i < l; i++) {
			  	if (fn(this, this.menu_items[i])) { //提供2个参数：菜单对象、当前的菜单项
					this.menu_items[i].hide();	//隐藏
				}else{
					this.menu_items[i].show();	//显示
				};
			};
	...
	$.fn.bindPopMenu = function(menu){
		this.bind('contextmenu', function(e){
			...
			menu.setTarget($(this)); //改变源后需要重新设置
	...
	MenuItem.prototype = {
		...
		createNormalItem : function(menu_item, parent_menu){
			...
			_this.$item.click(function (){		
				...
				//执行方法，目前提供3个参数：源，菜单项元素, 下标
				if(_this.fn) _this.fn(_this.parent_menu.target, _this.$item, _this.getSerialNumber());
		...
		hide : function(){
			if (this.$item.css('display') != 'none') { //注意不要用is方法来判断
				this.$item.hide();
				this.parent_menu.resizeMenu(this, 'delete');
			}
		},
		
		show : function(){
			if (this.$item.css('display') == 'none') {
				this.$item.show();
				this.parent_menu.resizeMenu(this, 'add');
			}
		},
